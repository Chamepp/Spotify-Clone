...
class APIFetchingEpisodes {
  ...
  func getEpisode(
    using endPoint: EpisodesEndpointInAPI,
  ...
  func getEpisodeDetails(
    ...
    AF.request(urlRequest)
      ...
      .responseDecodable(of: EpisodeDetailsResponse.self) { response in
        ...
        let episodeDetails = SpotifyModel.EpisodeDetails(
  ...
  private func fetchSavedEpisodesData(
    baseURL: String,
    accessToken: String,
    apiEndpoint: Utility.APIEndpoint,
    completionHandler: @escaping ([SpotifyModel.MediaItem]) -> Void
  ...
  private func parseEpisode(_ episode: Episode) -> SpotifyModel.MediaItem {
    ...
    let episodeDetails = SpotifyModel.EpisodeDetails(
      explicit: explicit, description: description, durationInMs: durationInMs,
